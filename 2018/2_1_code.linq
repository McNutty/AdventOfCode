...
void Main()
{
	...
	{
		if (StringHasMultipleCharacters(element, 2))
			twoCharacters++;
		if (StringHasMultipleCharacters(element, 3))
			threeCharacters++;
	}
	
	("Result: " + (twoCharacters*threeCharacters)).Dump();
}



bool StringHasMultipleCharacters(string line, int desiredOccurences)
{
	List<char> alphabet = new List<char>(){'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'};
	int occurences = 0;
	foreach (var element in alphabet)
	{
		occurences = line.Count(f => (f == element));
		if (occurences == desiredOccurences)
			break;
	}
	return (occurences == desiredOccurences);
}
